...
function onWinOpen(browserWindow) {
  console.log("onWinOpen");
  winLoad(utils.getMostRecentBrowserWindow());
}

function pageAttach(worker) {
  console.log("pageAttach");
  worker.port.on("selectableTextLink", function(payload) {
    workerPort = this; 
    downOnTextLink = payload[0];
    console.log("selectableTextLink: " + downOnTextLink);
    if (downWindow && downOnTextLink) {
      ...
      if (pref_changeCursor && (pref_selectGesture == selectGesture.hold || 
        ...
        holdTimeout = downWindow.setTimeout(function() {
          if (pref_selectGesture == selectGesture.hold) 
            changeCursor("text");
          else if (pref_selectGesture == selectGesture.immediate)
            changeCursor("grab");     
        }, pref_holdTimeMS);
      }
    }
  ...
  worker.on("pagehide", function(worker) { 
...
function onMouseDown(event) {
  ...
  if (event.button == 0 && event.detail != 0) {
    downEvent = event;  
    downWindow = utils.getMostRecentBrowserWindow();
    dragging = false;
    downOnTextLink = false;
  }
}  
...
function onMouseMove(event) {
  ...
  if (downWindow && downEvent) {
    ...
    if (Math.abs(event.clientX - downEvent.clientX) > pref_dragThresholdX || 
        Math.abs(event.clientY - downEvent.clientY) > pref_dragThresholdY) {
      ...
      var select = downEvent.ctrlKey || downEvent.shiftKey || downEvent.metaKey;
      // Drag method, default to horizontal
      if (!select) {
        if (pref_selectGesture == selectGesture.hold)
          select = (event.timeStamp - downEvent.timeStamp) > pref_holdTimeMS;
        else if (pref_selectGesture == selectGesture.immediate) 
          select = (event.timeStamp - downEvent.timeStamp) < pref_holdTimeMS;
        else   
          select = Math.abs(event.clientY - downEvent.clientY) < pref_dragThresholdY;
      }
      
      // Require window utils to generate native mouse event
      var wutils = downWindow.QueryInterface(Ci.nsIInterfaceRequestor)
                             .getInterface(Ci.nsIDOMWindowUtils);
      var mods = parseModifiers(downEvent); 
      // Make event coordinates relative to main window viewport
      var point = { x: downEvent.screenX - downWindow.mozInnerScreenX, 
